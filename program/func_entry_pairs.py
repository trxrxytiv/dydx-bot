    ...
    if len(series_1) > 0 and len(series_1) == len(series_2):
      ...
      if abs(z_score) >= ZSCORE_THRESH:
        ...
        if not is_base_open and not is_quote_open:
          ...
          accept_base_price = float(base_price) * 1.05 if z_score < 0 else float(base_price) * 0.95
          accept_quote_price = float(quote_price) * 1.05 if z_score > 0 else float(quote_price) * 0.95
